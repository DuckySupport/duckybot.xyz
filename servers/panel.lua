	...
	if cookie then
		if path[1] == "servers" and tonumber(path[2]) and path[3] == "panel" then
			...
			local function openPlayerPanel(playerData)
				...
				playerPanel.username.textContent = "@" .. playerData.name .. " (" .. string.format("%.0f", playerData.ID) .. ")"
			...
			local function searchPlayer()
				...
				if ERLC and ERLC.players then
					...
					for _, player in pairs(ERLC.players) do
						if player.roblox and (player.roblox.name:lower() == queryLower or player.roblox.displayName:lower() == queryLower or tostring(player.ID) == query) then
							openPlayerPanel({
			...
			local function renderShiftPanel(refreshQuota)
				...
				if Shifts.me.active then
                    ...
                    ]], formatTime(activeTime), formatTime(totalPauseTime), quotaBarHTML, onPause and "<span class=\"iconify text-xl\" data-icon=\"ion:play\"></span> Resume" or "<span class=\"iconify text-xl\" data-icon=\"f7:pause-fill\"></span> Pause")
					...
					shiftUpdateInterval = time.interval(1000, function()
						...
						if Shifts.me.active.paused then
							...
							Shifts.me.active.elapsed = (time.now() - Shifts.me.active.started) - Shifts.me.active.pausetime
						end
					...
					if type(Shifts.types) == "table" and next(Shifts.types) then
						...
						for _, shiftType in ipairs(Shifts.types) do
							optionsHTML = optionsHTML .. string.format("<button type=\"button\" data-value=\"%s\" class=\"w-full text-left px-4 py-2 text-sm text-white hover:bg-white/10\"><p>%s</p></button>", shiftType.name, shiftType.name)
						end
				...
				if User then
					if Guild then
						...
						if ERLC and ERLC.server then
							...
							elements.panel.glance.server.status.pill.classList:add("pill-red")
							elements.panel.glance.server.status.pill.innerHTML = "<span class=\"h-2 w-2 rounded-full\"></span> Not Linked"
